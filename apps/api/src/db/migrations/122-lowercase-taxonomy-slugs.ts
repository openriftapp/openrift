...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`ALTER TABLE card_domains     DROP CONSTRAINT card_domains_domain_slug_fkey`.execute(db);

  await sql`
    ALTER TABLE card_super_types DROP CONSTRAINT card_super_types_super_type_slug_fkey
  `.execute(db);

  await sql`UPDATE rarities    SET slug = lower(slug) WHERE slug <> lower(slug)`.execute(db);
  await sql`UPDATE card_types  SET slug = lower(slug) WHERE slug <> lower(slug)`.execute(db);
  await sql`UPDATE domains     SET slug = lower(slug) WHERE slug <> lower(slug)`.execute(db);
  await sql`UPDATE super_types SET slug = lower(slug) WHERE slug <> lower(slug)`.execute(db);
