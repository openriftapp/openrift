...
describe.skipIf(!ctx)("Card-sources query routes (integration)", () => {
  ...
  // ── stagingCandidatesForCard prefix-collision filter ──────────────────
  //
  // Regression: when one card's normalized name is a prefix/substring of
  // another card's, the shorter card's detail page used to pull in the
  // longer card's staging rows via LIKE-based fuzzy matching. The fix adds
  // a NOT EXISTS anti-join that defers to the card with the longer (more
  // specific) matching alias.

  describe("GET /admin/cards/:cardId — marketplaceStagingCandidates", () => {
    it("does not surface staging rows whose longer alias belongs to another card", async () => {
      const [shortCard] = await testDb
        .insertInto("cards")
        .values({
          slug: "CSQ-PX-SHORT",
          name: "CSQ Blast Cone",
      ...
      const [longCard] = await testDb
        .insertInto("cards")
        .values({
          slug: "CSQ-PX-LONG",
          name: "CSQ Blastcone Fae",
          type: "Unit",
          might: null,
          energy: 2,
          power: null,
      ...
      await testDb
        .insertInto("cardDomains")
        .values([
          { cardId: shortCard.id, domainSlug: "Mind", ordinal: 0 },
          { cardId: longCard.id, domainSlug: "Mind", ordinal: 0 },
        ])
        .execute();
      ...
      await testDb
        .insertInto("cardNameAliases")
        .values([
          { cardId: shortCard.id, normName: "csqblastcone" },
      ...
      const shortRes = await app.fetch(req("GET", "/admin/cards/CSQ-PX-SHORT"));
      expect(shortRes.status).toBe(200);
      const shortJson = await shortRes.json();
      const shortExternalIds = shortJson.marketplaceStagingCandidates.map(
        (r: { externalId: number }) => r.externalId,
      );
      expect(shortExternalIds).toContain(99_100);
      expect(shortExternalIds).not.toContain(99_101);

      const longRes = await app.fetch(req("GET", "/admin/cards/CSQ-PX-LONG"));
      expect(longRes.status).toBe(200);
      const longJson = await longRes.json();
      const longExternalIds = longJson.marketplaceStagingCandidates.map(
        (r: { externalId: number }) => r.externalId,
      );
      expect(longExternalIds).toContain(99_101);
      expect(longExternalIds).not.toContain(99_100);

      await testDb
        .deleteFrom("marketplaceStaging")
        .where("externalId", "in", [99_100, 99_101])
        .execute();
      await testDb
        .deleteFrom("cardNameAliases")
        .where("normName", "in", ["csqblastcone", "csqblastconefae"])
        .execute();
      await testDb
        .deleteFrom("cardDomains")
        .where("cardId", "in", [shortCard.id, longCard.id])
        .execute();
      await testDb.deleteFrom("cards").where("id", "in", [shortCard.id, longCard.id]).execute();
    });
  });
});
