...
- feat(Decks): **Legend Options for Neeko** — a deck with a card that lets you choose extra legends gets a Legend Options zone for them, and deck codes, text lists, share images and registration sheets carry them.
...
- fix(Decks): **Banned cards marked in image view** — deck cards in the image grid and stacked piles now carry the same Banned ribbon as the card browser.
...
- feat(Cards): **Labeled printing fan** — hovering a card with more than one printing now spines each one with its code and what sets it apart, such as its finish or size, instead of an unlabeled finish icon.
...
- fix(Meta): **Event pages in search results** — events with no standings are kept out of search, and the rest carry their venue and date in the title so same-named events tell apart.
- fix(Cards): **Card links in search and previews** — every printing of a card now points search engines at one page, links to renumbered printings redirect there, and share previews no longer show stray markdown.
...
- fix(Cards): **Rounded card art** — thumbnails were clipping to a square corner instead of the card's rounded shape.
...
- feat(Tournaments): **Withdrawn decks on the participants page** — players whose deck entry was withdrawn now carry a "Deck withdrawn" badge in the roster.
...
- fix(Collection): **Card changes stick when you return to the tab** — adding, moving, editing or removing cards right after returning to the tab no longer shows a false error, and the change no longer disappears when your collection refreshes in the background.
...
- feat(App): **Translations marked as beta** — a banner explains that translations are machine-made with a one-click way back to English, and offers your browser's other language when it is supported.
- feat(Meta Archive): **Filters as chips, like the card browser** — the archive front page, tournaments, legends and decks pages now show every filter as its own chip under the search field: era, tier, format, country, plus the page's own controls. The tournaments page gains a player-count range with quick presets and its "Archive holdings" picker sits in the same row, the front page gets a "With decklists" toggle, and the result count moved into the search field. On phones the chips fold into a drawer behind a filter button next to the search field, and your picks show as removable chips under it. Every option in every chip, the era included, shows how many tournaments it would leave.
...
- feat(Extension): **CardTrader prices in the Shopping Wizard** — the browser extension now marks a Cardmarket Shopping Wizard result the way it marks a seller's offers, with the cheapest CardTrader Zero price beside every row and a two-column summary comparing buying everything on Cardmarket, everything on CardTrader, or the cheapest per-seller mix, complete with shipping and an optional CardTrader Zero fee. Each side gets a ready wants list to copy or pick for OpenRift. The price beside a seller's offer is now always the CardTrader Zero price, the only one that says what the card costs elsewhere in euro; synchronize once more to pick it up.
...
- fix(Groups): **Every settled card lands in the inbox at once** — settling several trades in one go could show only the first card until the page was reopened; all of them now appear together.
