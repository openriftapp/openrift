...
describe("AttentionChangeList", () => {
  ...
  it("names a printing group by its catalog printing id", () => {
    renderList({ groups: [linkedPrintingGroup], ticked: new Set(["printing:cp2:artist"]) });
  ...
  it("names the printing and links to its full review when a handler is given", async () => {
    const onOpenNewPrinting = vi.fn();
    renderList({
      groups: [
        {
          ...newPrintingGroup,
          candidate: { id: "cp1" } as CandidatePrintingResponse,
        },
      ],
      ticked: new Set(["new-printing:cp1"]),
      onOpenNewPrinting,
    ...
    await userEvent.click(screen.getByRole("button", { name: "Review it below instead" }));
    expect(onOpenNewPrinting).toHaveBeenCalledWith("cp1");
    expect(screen.queryByText("foil · epic · Riot Artist")).not.toBeInTheDocument();
  });

  it("replaces the tick with the reason when the printing cannot be created as sent", () => {
    renderList({
      groups: [{ ...newPrintingGroup, candidate: { id: "cp1" } as CandidatePrintingResponse }],
      ticked: new Set(["new-printing:cp1"]),
      blockedNewPrintings: new Map([["cp1", "Language “JP” is not on the admin list"]]),
    });

    expect(screen.getByText(/Cannot be created as sent/u)).toBeInTheDocument();
