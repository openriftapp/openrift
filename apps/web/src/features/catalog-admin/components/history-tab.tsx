...
import type { HistoryFilter } from "@/features/catalog-admin/lib/history-events";
import {
  filterHistoryEvents,
  ...
  submissionOutcome,
} from "@/features/catalog-admin/lib/history-events";
...
function HistoryRow({ event, userId }: { event: AdminAuditEventResponse; userId: string | null }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {rows.length > 0 && (
        <CardList>
          {rows.map((event) => (
            <HistoryRow key={event.id} event={event} userId={userId} />
          ))}
        </CardList>
      )}
