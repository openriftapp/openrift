...
- **Segmented selectors** (one active option out of a joined row: group-by, time range, price source, validity filter) are `ToggleGroup` with `variant="outline" spacing={0}`, never a `ButtonGroup` whose active member switches to `variant="default"`. A cut fires on the active segment wherever it sits, so an interior selection loses its bottom-right corner with no outer edge to justify it, and `rounded-none` fights the group's end rounding. `ToggleGroup` marks the active segment with the control fill ladder (see "The control surface") and handles the first/last radii itself. Reference: the validity filter in `apps/web/src/features/decks/components/deck-list-toolbar.tsx`. The card-browser toolbars say `variant="control"` instead, to read as one family with the `control` Buttons beside them. On a `Toggle` the two variants resolve to the same fills.
- **Pairing rule:** next to a cut CTA, the lesser action is `ghost` (borderless, visibly a different species), not `outline`. `outline` carries the control fill in both themes and masquerades as a clashing rounded peer. Reserve `outline` for form-adjacent contexts away from cut buttons.
...
Text entry sits on the same rest fill, so `Input`, `Textarea`, `InputGroup`, `Select`, `Checkbox`, `RadioGroup` and `InputOTP` carry `bg-foreground/5` in both themes and mark disabled with opacity alone. `outline` runs the ladder as well, on `Button`, `Toggle` and `ToggleGroup` alike, so a bordered button, a segmented selector and the field beside them all sit on the same fill. `control` and `outline` differ only in intent: `control` says the element is a filter that can carry a value. `/admin/design/components` (Actions, Fields, Choice) renders every variant against every state, with the resolved colors measured live once Specs is on.
...
- **Selection** is `ring-2 ring-primary` for the chosen item and `ring-2 ring-primary/60` for a drop target, with `ring-offset-2 ring-offset-background` when the ring must clear an image. `border-primary` and `bg-primary/10` mark a chosen option inside a form, not a selected tile. Select mode is the exception: the signet (`SelectionMark`, `SelectionRowMark`) carries the picked state on its own, with no frame, no ring and nothing behind the tile.
...
- **A number** is a figure with a label. `StatStrip` items draw no edge. `StatTile` is for a number that navigates somewhere, and an overview page carries at most one `ActionBand`, the one waiting on the viewer. When an `ActionBand` value is a sentence rather than a count, `valueClassName` takes it off the display numeral.
...
`Dialog` is for a task, `AlertDialog` for a confirmation, `ConfirmActionDialog` for the shared destructive confirm, `Sheet` for a side panel, `Drawer` for a mobile bottom sheet. `DropdownMenu` is for actions, `Popover` for rich content, `Tooltip` for icon labels, `InfoHint` for a field explanation that must also open on touch, `HoverCard` for link previews, `ContextMenu` for right-click. Every overlay carries a title for screen readers.
