...
Text entry sits on the same rest fill, so `Input`, `Textarea`, `InputGroup`, `Select`, `Checkbox`, `RadioGroup` and `InputOTP` carry `bg-foreground/5` in both themes and mark disabled with opacity alone. `outline` runs the ladder as well, on `Button`, `Toggle` and `ToggleGroup` alike, so a bordered button, a segmented selector and the field beside them all sit on the same fill. `control` and `outline` differ only in intent: `control` says the element is a filter that can carry a value. `/admin/design` → Controls renders every variant against every state, with the resolved colors measured live.
