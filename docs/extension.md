...
3. On a seller's offers page, `annotate.content.ts` reads the stored snapshot, resolves each article row to a Cardmarket product id, and marks the row twice: a pill next to the product link for what the viewer owns and wants, and the CardTrader Zero price next to the seller's asking price. A row the snapshot does not cover gets neither, since a wrong `own 0` reads as a fact.

The seller's own asking price is coloured against that reference: green at or under it, amber up to a fifth over, red past that. Prices are read off the row with `parsePriceCents`, which handles both orders Cardmarket prints (`1.234,56 €` and `€1,234.56`), and the colour is an inline style on Cardmarket's own element, tracked by an attribute so a later pass can take it back.

The reference price is `cardtraderCents`: the cheapest CardTrader Zero listing for the printing behind the product, taken from the EN printing and falling back to the cheapest mapped printing. It is the only euro price OpenRift holds that stands for what the card costs elsewhere; Cardmarket's own price guide is site-wide and says nothing about language or seller country, so it is never used as a reference. Rows cover every mapped Cardmarket product, not only the ones the viewer owns or wants, which is why the popup counts only the latter when it reports what it holds.
...
The same content script runs on a Shopping Wizard result (`/Wants/ShoppingWizard/Results/<id>`), where Cardmarket lays out one card per seller with a table of the wanted articles. `cardmarketPageKind` in `cardmarket-url.ts` tells the two pages apart and `extractWizardRows` reads the tables: the product id comes from the same thumbnail tooltip, the quantity from the third cell, the name from `.card-name`, foil from the `.extras` icon, the language from the flag label, and the price from the last of Cardmarket's cells. The mobile copies of the rows carry no thumbnail and are skipped. The OpenRift column is a real `<td>` here, with a `<th>` per table and the help toggle on the first only.

`cardmarket-wizard.ts` adds what the offers page has no use for. Every seller card gets a line before its table saying how many of its rows CardTrader undercuts and by how much. The result summary at the top gets a panel with the wizard's article total, the total at the lower of the two shops per row, and two wants lists: the rows that stay with Cardmarket and the rows cheaper on CardTrader, each with a Copy button. `wizard-split.ts` holds the split and the text: a tie or a row with no CardTrader price stays with Cardmarket, the text is `2x Name` per line merged by name, and a moved foil or non-English row adds a note, since a wants list carries neither attribute. The Cardmarket list is meant to go through the wizard again, because fewer cards can pair up with other sellers, until nothing moves. "Pick for OpenRift" puts the CardTrader rows into the picks basket under the seller name `Shopping Wizard` at the wizard's quantity, so the popup's Send to OpenRift turns them into an organize list that keeps finish and language. The per-row `+`/`−` controls work on the same basket entry.

Shipping is left out of every figure on purpose: the wizard's shipping depends on its final selection, and CardTrader Zero charges one fee per order.
